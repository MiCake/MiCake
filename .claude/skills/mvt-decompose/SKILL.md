...
New-epic creation contract (`epic.yaml` is authoritative):
```yaml
version: 2
epic_id: "{epic_id}"
title: "{epic_title}"
...
requirement_context:
  ...
  sources:
    - id: "src-001"
      kind: "file" # file | conversation
      reference: "{normalized reference}"
      fingerprint: "sha256:{hex}" # file sources only
  items:
    - id: "ctx-001"
      category: "goal" # goal | in_scope | out_of_scope | business_rule | constraint | example | decision
      summary: "{self-contained requirement statement}"
      source_ids: ["src-001"]
  global_refs: ["ctx-001"]
children:
